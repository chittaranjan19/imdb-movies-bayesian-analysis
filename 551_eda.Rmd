...
```{r}
ggplot(data = revenue_month, aes(x = reorder(release_month,as.integer(release_month)), y = median_revenue)) + geom_bar(stat = "identity") + labs(title = "Median Opening Weekend Revenue per Month",x = "Month", y = "Median Opening Weekend Revenue")
```
...
```{r}
ggplot(subset(data, genres %in% c('Drama','Comedy','Horror')), aes(x=release_date, y=revenue, color = genres)) +
  geom_point() + 
  xlab("Date") + ylab("Revenue") + labs(title = "Largest Opening Revenues") +geom_text(data=subset(subset(data, genres %in% c('Drama','Comedy','Horror')),revenue>51000000),
...
```{r}
subset(data, genres %in% c('Drama','Comedy','Horror')) |>
  arrange(-revenue) |>
  select(c('revenue'))
```

```{r}
subset(subset(data, genres %in% c('Drama','Comedy','Horror')),revenue>51000000)
```
